...
const KawaiiChat = () => {
  ...
  const connectWebSocket = (tid) => {
    ...
    const ws = new WebSocket(`ws://xbxm.cloud:80/ws/chat?thread_id=${tid}`);
  ...
  useEffect(() => {
    const initChat = async () => {
      try {
        ...
        // 获取会话列表
        const listRes = await fetch('http://xbxm.cloud:80/chat/threads');
        ...
        if (threads.length > 0) {
          ...
          // 加载历史消息
          const historyRes = await fetch(`http://xbxm.cloud:80/chat/threads/${currentThreadId}/messages`);
  ...
  const createNewChat = async () => {
    try {
        console.log("🆕 正在创建新会话...");
        const createRes = await fetch('http://xbxm.cloud:80/chat/threads', {
            method: 'POST',
  ...
  const handleSend = async () => {
    ...
    try {
      // 使用流式接口发送消息
      const response = await fetch(`http://xbxm.cloud:80/chat/threads/${threadId}/text-messages-stream`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
