...
export function useWebRTC(initialRoomId, onRemoteAudio, onError) {
  ...
  const ensureSignaling = useCallback((roomId) => {
    ...
    try {
      const socket = new WebSocket(`${WS_PREFIX}/ws/webrtc/${roomId}`);
      socket.onclose = () => {
  ...
  const sendCandidate = useCallback(async (roomId, candidate) => {
    try {
      await fetch(`${API_PREFIX}/webrtc/${roomId}/candidate`, {
        method: "POST",
  ...
  const stopCall = useCallback(() => {
    ...
    if (activeRoomId) {
      fetch(`${API_PREFIX}/webrtc/${activeRoomId}`, {
        method: "DELETE",
  ...
  const startCall = useCallback(
    async ({ roomId: explicitRoomId, mode = "voice" } = {}) => {
      ...
      try {
        ...
        const response = await fetch(`${API_PREFIX}/webrtc/${callRoomId}/offer`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            type: "offer",
        ...
        try {
          const stateResponse = await fetch(`${API_PREFIX}/webrtc/${callRoomId}`);
          if (stateResponse.ok) {
