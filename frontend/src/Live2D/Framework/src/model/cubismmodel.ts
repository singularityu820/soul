...
function getCubismBlendMode() {
  if (!cubismrenderer || !cubismrenderer.CubismBlendMode) {
    throw new Error('CubismBlendMode 未正确加载。请检查模块依赖。');
  }
  return cubismrenderer.CubismBlendMode;
}
...
function getCsmMap() {
  if (!csmmap || !csmmap.csmMap) {
    throw new Error('csmMap 未正确加载。请检查模块依赖。');
  }
  return csmmap.csmMap;
}

// 类型定义 - 使用命名空间访问类型
// 注意：这些类型在编译时使用，运行时通过辅助函数访问值
// 由于循环依赖，使用 typeof 来获取类型，避免在模块解析时访问 undefined
type CubismBlendMode = typeof cubismrenderer.CubismBlendMode[keyof typeof cubismrenderer.CubismBlendMode];
type csmVector<T> = typeof csmvector.csmVector extends new (...args: any[]) => infer R ? R : any;
type csmMap<K, V> = typeof csmmap.csmMap extends new (...args: any[]) => infer R ? R : any;
...
export namespace Live2DCubismFramework {
  ...
  export class CubismModel {
    ...
    public getDrawableId(drawableIndex: number): CubismIdHandle {
      ...
      return getCubismFramework().getIdManager().getId(parameterIds[drawableIndex]);
    }
    ...
    public getDrawableBlendMode(drawableIndex: number): CubismBlendMode {
      ...
      return Live2DCubismCore.Utils.hasBlendAdditiveBit(
        ...
        ? getCubismBlendMode().CubismBlendMode_Additive
        : Live2DCubismCore.Utils.hasBlendMultiplicativeBit(
            constantFlags[drawableIndex]
          )
        ? getCubismBlendMode().CubismBlendMode_Multiplicative
        : getCubismBlendMode().CubismBlendMode_Normal;
    }
...

      {
        ...
        for (let i = 0; i < parameterCount; ++i) {
          this._parameterIds.pushBack(
            getCubismFramework().getIdManager().getId(parameterIds[i])
          );
        }
      }

      {
        ...
        for (let i = 0; i < partCount; ++i) {
          this._partIds.pushBack(
            getCubismFramework().getIdManager().getId(partIds[i])
          );
        }
      }

      {
        ...
        for (let i = 0; i < drawableCount; ++i) {
          this._drawableIds.pushBack(
            getCubismFramework().getIdManager().getId(drawableIds[i])
          );
        }
      }
    }
    ...
    public constructor(model: Live2DCubismCore.Model) {
      ...
      this._savedParameters = new (getCsmVector())<number>();
      this._parameterIds = new (getCsmVector())<CubismIdHandle>();
      this._drawableIds = new (getCsmVector())<CubismIdHandle>();
      this._partIds = new (getCsmVector())<CubismIdHandle>();

      this._notExistPartId = new (getCsmMap())<CubismIdHandle, number>();
      this._notExistParameterId = new (getCsmMap())<CubismIdHandle, number>();
      this._notExistParameterValues = new (getCsmMap())<number, number>();
      this._notExistPartOpacities = new (getCsmMap())<number, number>();
    }
