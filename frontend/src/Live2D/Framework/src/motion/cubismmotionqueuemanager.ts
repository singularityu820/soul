...
function getCubismModel() {
  if (!cubismmodel || !cubismmodel.CubismModel) {
    throw new Error('CubismModel 未正确加载。请检查模块依赖。');
  }
  return cubismmodel.CubismModel;
}
...
function getIterator() {
  if (!csmvector || !csmvector.iterator) {
    throw new Error('iterator 未正确加载。请检查模块依赖。');
  }
  return csmvector.iterator;
}

function getCubismMotionQueueEntry() {
  if (!cubismmotionqueueentry || !cubismmotionqueueentry.CubismMotionQueueEntry) {
    throw new Error('CubismMotionQueueEntry 未正确加载。请检查模块依赖。');
  }
  return cubismmotionqueueentry.CubismMotionQueueEntry;
}
...
type CubismModel = typeof cubismmodel.CubismModel extends new (...args: any[]) => infer R ? R : any;
type csmVector<T> = typeof csmvector.csmVector extends new (...args: any[]) => infer R ? R : any;
type iterator<T> = typeof csmvector.iterator extends new (...args: any[]) => infer R ? R : any;
type CubismMotionQueueEntry = typeof cubismmotionqueueentry.CubismMotionQueueEntry extends new (...args: any[]) => infer R ? R : any;
...
export namespace Live2DCubismFramework {
  ...
  export class CubismMotionQueueManager {
    ...
    public constructor() {
      ...
      this._motions = new (getCsmVector())<CubismMotionQueueEntry>();
    }
    ...
    public startMotion(
      ...
      motionQueueEntry = new (getCubismMotionQueueEntry())(); // 退出时丢弃
