...
export class LAppLive2DManager {
   ...
   * マウス移動時の処理（目の向き用）
   *
   * @param x 画面のX座標（-1から1の範囲に正規化）
   * @param y 画面のY座標（-1から1の範囲に正規化）
   */
  ...
  public onTap(x: number, y: number): void {
    ...
    for (let i = 0; i < this._models.getSize(); i++) {
      ...
      const hitAreasCount = modelSetting.getHitAreasCount();
      let hitHead = false;
      let hitBody = false;
      let headAreaName: string | null = null;
      let bodyAreaName: string | null = null;
      
      // 查找实际的头部和身体区域名称（支持大小写变化）
      for (let j = 0; j < hitAreasCount; j++) {
        const areaName = modelSetting.getHitAreaName(j);
        const areaNameLower = areaName.toLowerCase();
        
        // 检查是否是头部区域（支持 "Head", "head", "HEAD" 等）
        if (areaNameLower === 'head') {
          headAreaName = areaName;
        }
        // 检查是否是身体区域（支持 "Body", "body", "BODY" 等）
        else if (areaNameLower === 'body') {
          bodyAreaName = areaName;
        }
      }
      
      // 使用找到的区域名称进行检测，如果没找到则使用默认名称
      if (headAreaName) {
        hitHead = model.hitTest(headAreaName, x, y);
      } else {
        hitHead = model.hitTest(LAppDefine.HitAreaNameHead, x, y);
        headAreaName = LAppDefine.HitAreaNameHead;
      }
      
      if (bodyAreaName) {
        hitBody = model.hitTest(bodyAreaName, x, y);
      } else {
        hitBody = model.hitTest(LAppDefine.HitAreaNameBody, x, y);
        bodyAreaName = LAppDefine.HitAreaNameBody;
      }
      
      if (LAppDefine.DebugLogEnable) {
        LAppPal.printMessage(`[APP]Available hit areas count: ${hitAreasCount}`);
        for (let j = 0; j < hitAreasCount; j++) {
          const areaName = modelSetting.getHitAreaName(j);
          LAppPal.printMessage(`[APP]Hit area ${j}: "${areaName}"`);
        }
        LAppPal.printMessage(`[APP]Head hit test (${headAreaName}): ${hitHead}`);
        LAppPal.printMessage(`[APP]Body hit test (${bodyAreaName}): ${hitBody}`);
      }
      
      if (hitHead) {
        if (LAppDefine.DebugLogEnable) {
          LAppPal.printMessage(
            `[APP]hit area: [${headAreaName}]`
          );
        }
        this.talkPrint(LAppDefine.HitHeadList[Math.floor(Math.random()*LAppDefine.HitHeadList.length)]);
        model.setRandomExpression();
        continue; // 头部点击后不再检查其他区域
      }
      
      if (hitBody) {
        if (LAppDefine.DebugLogEnable) {
          LAppPal.printMessage(
            `[APP]hit area: [${bodyAreaName}]`
          );
        }
