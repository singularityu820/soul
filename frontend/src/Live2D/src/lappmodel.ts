...
export class LAppModel extends CubismUserModel {
  ...
  public update(): void {
    ...
    // 目の向きの調整（优先使用鼠标位置，如果没有则使用拖动位置）
    const eyeX = this._mouseX !== undefined ? this._mouseX : this._dragX;
    const eyeY = this._mouseY !== undefined ? this._mouseY : this._dragY;
    // 使用 addParameterValueById 累加值，确保眼睛跟随鼠标
    this._model.addParameterValueById(this._idParamEyeBallX, eyeX); // -1から1の値を加える
    this._model.addParameterValueById(this._idParamEyeBallY, eyeY);
    
    // 头部跟随鼠标（幅度比眼睛小，使效果更自然）
    if (this._mouseX !== undefined && this._mouseY !== undefined) {
      // 头部转动的幅度约为眼睛的 15 倍（因为眼睛参数范围是 -1 到 1，头部参数范围更大）
      const headSensitivity = 120; // 头部跟随的灵敏度，可以调整
      this._model.addParameterValueById(this._idParamAngleX, this._mouseX * headSensitivity);
      this._model.addParameterValueById(this._idParamAngleY, this._mouseY * headSensitivity);
      this._model.addParameterValueById(
        this._idParamAngleZ,
        this._mouseX * this._mouseY * -headSensitivity
      );
    }
  ...
  /**
   * マウス位置を設定する（目の向き用）
   * @param x マウスX座標（-1から1の範囲）
   * @param y マウスY座標（-1から1の範囲）
   */
  ...
  public hitTest(hitArenaName: string, x: number, y: number): boolean {
    ...
    if (this._opacity < 1) {
      if (LAppDefine.DebugLogEnable) {
        LAppPal.printMessage(`[APP]hitTest failed: model opacity < 1 (${this._opacity})`);
      }
      return false;
    }
    ...
    for (let i = 0; i < count; i++) {
      if (this._modelSetting.getHitAreaName(i) == hitArenaName) {
        ...
        const result = this.isHit(drawId, x, y);
        
        if (LAppDefine.DebugLogEnable) {
          LAppPal.printMessage(`[APP]hitTest for "${hitArenaName}": input (${x.toFixed(2)}, ${y.toFixed(2)}), result: ${result}`);
        }
    ...
    if (LAppDefine.DebugLogEnable) {
      LAppPal.printMessage(`[APP]hitTest: hit area "${hitArenaName}" not found`);
    }
    return false;
  }
  ...
  _mouseX: number | undefined; // マウスX座標（目の向き用）
  _mouseY: number | undefined; // マウスY座標（目の向き用）
