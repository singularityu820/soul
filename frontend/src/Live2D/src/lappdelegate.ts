  ...
  public initialize(): boolean {
    ...
    document.addEventListener("mousemove", function (e) {
      ...
      if(document.getElementById("live2d")){
        ...
        // 眼睛跟随鼠标：将鼠标位置相对于画布中心归一化到 -1 到 1 的范围
        const live2DManager = LAppLive2DManager.getInstance();
        const canvasWidth = rect.width;
        const canvasHeight = rect.height;
        
        // 将鼠标位置转换为相对于画布中心的坐标（-1 到 1）
        // X: 左边是 -1，中心是 0，右边是 1
        // Y: 上边是 1，中心是 0，下边是 -1（注意屏幕坐标 Y 向下，所以需要反转）
        let normalizedX = ((posX / canvasWidth) - 0.5) * 2; // -1 到 1
        let normalizedY = ((posY / canvasHeight) - 0.5) * -2; // -1 到 1，Y 轴反转
        
        // 应用缩放因子使眼睛移动更明显（可以根据需要调整）
        const eyeSensitivity = 0.5; // 灵敏度，值越小眼睛移动越明显
        normalizedX *= eyeSensitivity;
        normalizedY *= eyeSensitivity;
        
        // 限制在 -1 到 1 范围内
        const eyeX = Math.max(-1, Math.min(1, normalizedX));
        const eyeY = Math.max(-1, Math.min(1, normalizedY));
        
        live2DManager.onMouseMove(eyeX, eyeY);
      }
    ...
    document.addEventListener("mouseout", function (e) {
      ...
      live2DManager.onMouseMove(0.0, 0.0); // 眼睛也归位
    }, false);
    ...
    canvas.addEventListener("click", function (e) {
      ...
      if (LAppDefine.DebugLogEnable) {
        LAppPal.printMessage(`[APP]Canvas click - client: (${e.clientX}, ${e.clientY}), rect: (${rect.left}, ${rect.top}), pos: (${posX.toFixed(2)}, ${posY.toFixed(2)})`);
      }
