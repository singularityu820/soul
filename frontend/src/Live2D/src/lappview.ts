...
export class LAppView {
  ...
  public initialize(): void {
    ...
    if (LAppDefine.DebugLogEnable) {
      LAppPal.printMessage(`[APP]DeviceToScreen init - width: ${width}, height: ${height}, screenW: ${screenW}, left: ${left}, right: ${right}`);
    }
  ...
  public onTouchesEnded(pointX: number, pointY: number): void {
    ...
    {
      // シングルタップ
      const deviceX = this._touchManager.getX();
      const deviceY = this._touchManager.getY();
      // 使用 transformViewX/Y 进行完整的坐标转换（包括 deviceToScreen 和 viewMatrix）
      const x: number = this.transformViewX(deviceX); // 論理座標変換した座標を取得。
      const y: number = this.transformViewY(deviceY); // 論理座標変化した座標を取得。

      if (LAppDefine.DebugLogEnable) {
        const screenX = this._deviceToScreen.transformX(deviceX);
        const screenY = this._deviceToScreen.transformY(deviceY);
        const { width, height } = canvas;
        const matrix = this._deviceToScreen.getArray();
        LAppPal.printMessage(`[APP]onTouchesEnded - canvas: (${width}, ${height}), device: (${deviceX.toFixed(2)}, ${deviceY.toFixed(2)})`);
        LAppPal.printMessage(`[APP]Matrix - scaleX: ${matrix[0].toFixed(4)}, scaleY: ${matrix[5].toFixed(4)}, transX: ${matrix[12].toFixed(2)}, transY: ${matrix[13].toFixed(2)}`);
        LAppPal.printMessage(`[APP]Coordinates - screen: (${screenX.toFixed(2)}, ${screenY.toFixed(2)}), view: (${x.toFixed(2)}, ${y.toFixed(2)})`);
      }
