...
class SoVITSTTSClient(BaseTTSClient):
    ...
    async def synthesize_stream(self, text: str, voice: str, locale: str) -> AsyncIterator[str]:
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.info(f"SoVITS TTS Stream: Split text into {len(chunks)} chunks")
        
        async with self._lock:
            for i, chunk in enumerate(chunks):
                try:
                    url = await self._request_chunk(chunk.text, voice, locale)
                    if url:
                        logger.info(f"SoVITS TTS Stream: Chunk {i+1}/{len(chunks)} completed")
                        yield url
                except Exception as e:
                    logger.error(f"SoVITS TTS Stream: Chunk {i+1} failed: {e}")
                    raise
...
class TTSService:
    ...
    async def synthesize_stream(self, text: str, voice: str, locale: str) -> AsyncIterator[str]:
        ...
        # 对于支持流式合成的客户端，直接使用流式方法
        if hasattr(client, "synthesize_stream"):
            async for url in client.synthesize_stream(text, voice, locale):
                yield url
        else:
            # 对于不支持流式合成的客户端，使用普通合成并包装成流式
            result = await client.synthesize(text, voice, locale)
            if result.audio_reference:
                yield result.audio_reference
            if result.segments:
                for segment in result.segments:
                    if segment and segment != result.audio_reference:
                        yield segment
